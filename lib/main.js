...
program.command('assets <project-dir> <assets-folder>')
    ...
    .action(function AssetsCommand(projectDir, assetFolder, options) {
        projectDir = normalizeDir(projectDir);    
        ...
        console.log('TiSpork assets: processing folder ' + assetFolder);
        ...
        fs.readdir(assetsDir, function(error, curFiles) {
            ...
            for (var i = 0; i < curFiles.length; i++) {
                ...
                if (program.android) {
                    execOptions = { cwd: getRootDir(projectDir) + 'android/images/', debug: program.debug };
                    if (noScale) 
                        cmds.push({ cmd: 'convert ' + inFile + ' -resize 100% '  + assetFolder + '/' + curFiles[i], options: execOptions });
                    if (options.three)
                        cmds.push({ cmd: 'convert ' + inFile + ' -resize 37% low/' + assetFolder + '/' + curFiles[i], options: execOptions });
                    if (options.four)
                        cmds.push({ cmd: 'convert ' + inFile + ' -resize 50% medium/' + assetFolder + '/' + curFiles[i], options: execOptions });
                    if (options.six)
                        cmds.push({ cmd: 'convert ' + inFile + ' -resize 75% high/' + assetFolder + '/' + curFiles[i], options: execOptions });
                    if (options.eight)
                        cmds.push({ cmd: 'convert ' + inFile + ' -resize 100% res-xhdpi/' + assetFolder + '/' + curFiles[i], options: execOptions });
                }
                
                if (program.ios) {
                    execOptions = { cwd: getRootDir(projectDir) + 'iphone/images/' + assetFolder + '/', debug: program.debug };
                    if (options.retina) {
                        cmds.push({ cmd: 'convert ' + inFile + ' -resize 50% ' + curFiles[i], options: execOptions });
                        cmds.push({ cmd: 'convert ' + inFile + ' -resize 100% ' + fileName + '@2x.' + fileType, options: execOptions });
                    } else {
                        cmds.push({ cmd: 'convert ' + inFile + ' -resize 100% ' + curFiles[i], options: execOptions });
                    }
                }
